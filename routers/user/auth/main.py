...
@router.post("/send-password-reset-code", name='Password Reset Code')
async def request_password_reset_code(background_tasks:BackgroundTasks, payload:schemas.UserCode, db:Session=Depends(get_db)):
    obj = await crud.verify_admin_user_code_add_pass_reset_code(payload, db)
    scheduler.add_job(
        crud.del_code,
        trigger='date',
        kwargs={'_type':'pass-reset-code', 'pk':obj[0].user_id},
